...
def getmax_identity(accumulated, current, ua_cmp=lambda x:x, email_cmp=lambda x:x):
    """
    ...
    """
    email = email_cmp(current['author__email'])
    if email and len(email)>accumulated:
        return len(current['author__email'])
