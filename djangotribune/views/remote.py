...
class RemoteBaseMixin(ChannelAwareMixin):
    ...
    remote_fields = ('clock', 'created', 'author__email', 'user_agent', 'raw')
...
class RemotePlainMixin(RemoteBaseMixin):
    ...
    def build_backend(self, messages):
        ...
        * Identity column width is aligned on the biggest identity (email or 
          user_agent);
        * Message column takes the size left in the table maximum width.
        ...
        email_modifier = lambda x: u"<{0}>".format(x)
        identity_col_width = reduce(lambda x,y: getmax_identity(x,y, email_cmp=email_modifier), messages, 10)
        ...
        for item in messages:
            ...
            if item['author__email']:
                identity = email_modifier(item['author__email'])
...
class RemoteJsonMixin(RemoteBaseMixin):
    ...
    remote_fields = ('id', 'created', 'clock', 'author__email', 'user_agent', 'web_render')
    default_row_direction = "asc"
    ...
    def patch_row(self, row):
        ...
        row['owned'] = False
        if (self.request.user.is_authenticated() and row['author__email'] and self.request.user.email == row['author__email']) or (row['id'] in self.user_owned_messages):
            row['owned'] = True
        return row
...
class RemoteXmlMixin(RemoteBaseMixin):
    ...
    remote_fields = ('id', 'created', 'author__email', 'user_agent', 'remote_render')
    ...
    def build_backend(self, messages):
        ...
        for item in messages:
            ...
            email = ET.SubElement(message_item, "login")
            if item['author__email']:
                email.text = item['author__email']
