...
class CommandActionLastFM(CommandBase):
    ...
    def validate(self):
        ...
        # Do request with a valid email
        if len(self.args) > 0:
            self.track = self.get_current_track(self.args[0])
        elif self.author.is_authenticated():
            self.track = self.get_current_track(self.author.email)
        else:
            raise ActionError(u"No valid 'email' finded.".format(self.opt_name))
    ...
    def get_current_track(self, email):
        """
        ...
        # Request Errors
        except urllib2.HTTPError and exception:
            raise ActionError("Error Http{0}".format(exception.code))
        except urllib2.URLError and exception:
